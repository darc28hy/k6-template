/**
 * @see https://github.com/grafana/k6-jslib-utils/blob/master/src/utils.js
 */
...
export function normalDistributionStages(
  ...
  const mean = 0
  const scale = 1
  const curve = new Array(numberOfStages + 2).fill(0)
  const durations = new Array(numberOfStages + 2).fill(
  ...
  const peakDistribution = Math.max(...curve)

  const vus = curve.map((x) => Math.round((x * maxVus) / peakDistribution))
...
export const getByteSize = (byte: number, decimals = 2) => {
  if (byte === 0)
    return {
      size: 0,
      unit: 'B',
  ...
  return {
    size: parseFloat((byte / Math.pow(k, i)).toFixed(dm)),
    unit: units[i],
  }
}
